...
- `scripts/voxtype-rephrase` — reads PRIMARY selection via xclip, rewrites via Groq, pastes via `voxtype-paste-active`.
...
`voxtype-rephrase` — binds to Super+R. Reads PRIMARY selection, rewrites via Groq (15s timeout, temp 0.3), pastes via `voxtype-paste-active`. Short snippets <40 chars or code patterns skip the LLM. System prompt overridable via `REPHRASE_STYLE` env var.

```
bind = $main R, exec, ~/.local/bin/voxtype-rephrase    # Hyprland
```

## Terminal detection

`scripts/voxtype-paste-active:31` and `scripts/voxtype-rephrase:154` maintain duplicate terminal match lists (`kitty`, `alacritty`, `ghostty`, `wezterm`, `konsole`, `ptyxis`, `kgx`, `tilix`, `terminal`, `console`). **Keep in sync** when adding a new terminal.
