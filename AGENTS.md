...
- Runtime dictation flow is `voxtype daemon -> voxtype-clean-dictation -> clipboard -> voxtype-paste-active`. Rewrite and summarize hotkeys share the bar runner (`relay-bar --action`).
...
- Bar actions (rewrite/summarize/custom) use `_voxtype_groq.token_budget()` for the Groq 8000 TPM limit; rewrite caps completion at 1536. Do not replace dictation's fixed cap with this larger dynamic budget.
- Direct hotkeys Ctrl+Alt+R / Ctrl+Alt+S call `relay-bar --action rewrite|summarize` (shared runner + preview). `voxtype-rephrase` / `voxtype-summarize` are thin wrappers for install compatibility.
- Keep terminal detection markers in `scripts/voxtype-paste-active`; terminals require Ctrl+Shift+V rather than Ctrl+V.
