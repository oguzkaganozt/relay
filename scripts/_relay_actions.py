...
Shared by relay-bar (UI) and direct hotkey runners so the action list and
the context-line format can't drift.
...
#   dictation  -> launches the dictation flow
#   transform  -> rewrites source text, Preview + Apply
#   result     -> returns a result card, Copy
#   custom     -> free-form text transform/analysis (instruction required)
ACTIONS: list[dict] = [
...
# ---- common action model (prompts + runner) ----
# kind maps to the V2 result behavior:
#   transform -> Preview + Apply (rewrite)
#   result    -> result card + Copy (summarize)
#   custom    -> preview + Apply (free-form text only; no external effect)
#
...
ACTION_SPECS: dict[str, dict] = {
    ...
    "custom": {
        "system": (
            ...
            "Follow the user's Instruction below. "
...
def run_action(action_id: str, input_text: str, groq_module,
    ...
    if ACTION_SPECS[action_id].get("needs_instruction") and not (instruction or "").strip():
        return None, "instruction required"
